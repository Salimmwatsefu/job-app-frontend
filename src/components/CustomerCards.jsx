import React, { useRef, useEffect } from 'react';

function CustomerCards() {
  const carouselRef = useRef(null);

  useEffect(() => {
    const interval = setInterval(() => {
      carouselRef.current.scrollLeft += 300; // Adjust the scroll distance as needed
    }, 3000); // Autoplay interval in milliseconds (e.g., 3000 = 3 seconds)

    return () => {
      clearInterval(interval);
    };
  }, []);

  return (
    <div className=''>
      <div>
        <h1 className='flex justify-center font-semibold md:mt-20 mt-5 text-3xl'>Our Customers</h1>
        <p className='justify-center flex md:mt-5 mt-3 md:mx-0 mx-4 text-gray-500'>
          Our customers are consistently impressed with our service and have formed loyal relationships with us
        </p>
      </div>
      <div className='flex flex-wrap md:gap-16 justify-center items-center mt-14 mb-28 md:mx-0 mx-3  overflow-x-auto' ref={carouselRef}>
        <img
          src='https://www.sap.com/dam/application/shared/logos/sap-logo-svg.svg/sap-logo-svg.svg'
          className='w-28'
        />
        {/*<img src='https://neojb.brickthemes.com/wp-content/uploads/2018/08/logo_02.png' className='w-40'/>*/}
        <img src='https://www2.deloitte.com/content/dam/assets/logos/deloitte.svg' className='w-40 bg-black' />
        <img src='https://www.nasa.gov/sites/all/themes/custom/nasatwo/images/nasa-logo.svg' className='w-28' />
        <img src='data:image/png;base64,' className='w-28 hidden md:block'/>
        <img src='https://img-prod-cms-rt-microsoft-com.akamaized.net/cms/api/am/imageFileData/RE1Mu3b?ver=5c31' className='w-40'/>
            
        </div>
        </div>
        );
        }

export default CustomerCards

